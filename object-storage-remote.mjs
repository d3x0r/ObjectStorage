...
export function ObjectStorage( ws ) {
	...
	function objectStorageContainer(o,opts) {
		...
		/*
			this.data = {
				//nonce : opts?opts.sign?SaltyRNG.sign( JSOX.stringify(o), 3, 3 ):null:null,
				data : o
			}
		*/
			if( opts && opts.sign ) {
				var v = this_.SaltyRNG.verify( JSOX.stringify(o), this.data.nonce, 3, 3 );
				//console.log( "TEST:", v );
	...
	newStorage. remote = {
		get(opts){
                    ...
                    const existing = localStrorage.getItem( opts.id );
                    if( existing ) return Promise.resolve( existing );

                    if( this.ws.length )
			return new Promise( (res,rej)=>{
				const msg = {op:"get", id:unique++, opts:opts, res:res,rej:rej, idx:undefined };
				this.ws[0].send( msg );
                                msg.idx = 0;
		...
		put(obj, rid,res,rej){
				...
				requests.set( msg.id, msg );
                              ...
                              if( !opts.id ) opts.id = SaltyRNG.id();
                              localStorage.setItem( opts.id, JSOX.stringify( obj ) );
	...
	return new Promise( function(res,rej){
		...
		if( opts && opts.id ) {
			...
			_debug_osr && console.log( "New bare object, create a container...", opts );
                        if( !opts ) opts = { id : SaltyRNG.id() }
						else opts.id = SaltyRNG.id();
...
ObjectStorage.prototype.connect = function(remote) {
    // assign websocket transport to storage object
        this.ws.push( remote );
        remote.storage = this;
}

ObjectStorage.prototype.disconnect = function(remote) {
